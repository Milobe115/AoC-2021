...
class board = object (self) 
  ...
  method calculate_score final_number =
    ...
    Printf.printf "Score final avant mult par la valeur %d : %d \n" final_number !temp_score;
...
let rec final_process numbers board =
  match numbers with
  | [] -> -1
  | hd::tl -> board#check_value hd;
              if board#check_board then
                board#calculate_score hd
              else
                final_process tl board
  
let rec process_numbers numbers (boards: board list) =
  let rec check_boards boards last_number = match boards with
    | [] -> []
    | hd::tl -> if hd#check_board then
                  check_boards tl last_number
                else
                  (hd::check_boards tl last_number) in
  match numbers with
  | [] -> -1
  | hd::tl -> List.iter (fun x -> x#check_value hd) boards;
              let new_boards = check_boards boards hd in
              match new_boards with
              | [] -> raise Error
              | [elt] -> final_process tl elt
              | _ -> process_numbers tl new_boards
;;
